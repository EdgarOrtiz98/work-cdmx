...
function indefinido()
 ...
 doc = document.clsf;
 	...
 	{
 	doc.vigenciaac.value = "<%= strFechaAC %>";
 	doc.vigenciaat.value = "<%= strFechaAT %>";
 	doc.vigenciatot.value = "<%= strFechaTotal %>";
...
function habilita2()
	{
		...
		{
		document.clsf.fecinact.value = "<%=horafec%>";
...
<INPUT type="hidden" value="<%=stridSeccion%>" name="SerieP2">
<%if(valida == 2){
		strsoloDesc = "1";
	}
	%>
			...
			<TD width="125"><a href="<%= retURI %>"> <IMG
				...
				<%if(vertodo=="true") {%>
						<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
							<a></a>
						<%	}else{%>	
							<a><img	src="../Imagenes/Save.gif" title="Guardar" border="0"	onclick="GuardarClasf()"></a> 
						<%}%>
				<%}%>
			</TD>
			<TD width="125">
...
<%if(vertodo=="true") {%>
		...
		<TR valign="top">
			...
			<TD width="528">
				<INPUT name="struct" value="<%=strstruc%>" MAXLENGTH="200" type="text" class="blue500" 
					<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
						disabled="disabled"
					<%	}%>	
					<%if(valida == 2) 
		...
		<TR valign="top">
			...
			<%if(valida == 2 || (strInact.trim().length()>0 && Integer.parseInt(strInact) == 3)  ){%>
					<%if (strIdSubfondo != null && strIdSubfondo.trim().length()>0){%>
					<meve_combo:comboseleccion
						data="<%= ComboData.getValorCombo() %>" size="1" name="valor"
		...
		<TR valign="top">
			...
			<INPUT name="vigenciaat" 
					...
					<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
						disabled="disabled"
					<%	}%>												
			value="<%=strFechaAT%>" MAXLENGTH="2" type="text" class="blue100" onblur="sum();"
