	...
	<TBODY>
		<TR>
			<TD width="43%" class="etiquetas">
				...
				<A title="<%=ActualizaTipo_Doc.getNombreTipoDocto((String)vTiene.get(i))%>">
				&nbsp;&nbsp;** <%=Utilerias.recortaString(ActualizaTipo_Doc.getNombreTipoDocto((String)vTiene.get(i)),80)%> **&nbsp;&nbsp;</A> tiene <%=(String)vTiene.get(i+1)%><BR>
				<%}%>
				</FONT>
			</TD>
		</TR>
...
<TABLE cellspacing="0" cellpadding="0" width="100%" border="0">
	<TBODY>
		<TR valign="top">
			<TD width="40%"><A href="TipoDocumento.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" 
				target="_self"> <IMG
...
%>
	...
	<% t++; %>
	<TR <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="50%">
    			<INPUT type=checkbox value="<%=campo2%>" name="Borrar">
    			<FONT face=Arial color=#000080 size=1>
		    	<A href="TipoDocumento.jsp?id_tipo_doc=<%=campo2%>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" 
					title="<%=campo1%>"><script> document.write(trunc('<%=campo1%>','Asunto_HD'));</script></A>
    			</FONT>
		</TD>
		<TD	width="50%" align="center">
			<FONT face=Arial color=#000080 size=1><A title="<%= campo4 %>"><script> document.write(trunc('<%=campo4%>','Asunto_HD2'));</script> </A></FONT>
		</TD>
	</TR>
	<% j++; %>
	<% } %>
