...
<%
	...
	if (strAccion != null && strAccion.trim().length() > 0 && strAccion.equals("borrar")){
		vTiene	=	ActualizaTipoAsunto.deleteTipoAsunto(strIds_Borrar, BUsuario.getIdUsuario());
	...
	String sql[][] = ActualizaTipoAsunto.getListaTipoAsuntos(strOrden, strTipo);
...
<FORM action="TipoAsuntoView.jsp" name="TipoAsuntoView" method="get">
	...
	<TBODY>
		<TR>
			<TD width="43%" class="etiquetas">
				...
				&nbsp;&nbsp;No se pueden eliminar los siguientes Tipos de Asunto (razones)<BR><BR>
				<%for(int i=0;i<vTiene.size();i=i+2){%>
				<A title="<%=ActualizaTipoAsunto.getNombreTipoAsunto((String)vTiene.get(i))%>">
				&nbsp;&nbsp;** <%=Utilerias.recortaString(ActualizaTipoAsunto.getNombreTipoAsunto((String)vTiene.get(i)),80)%> **&nbsp;&nbsp;</A> tiene <%=(String)vTiene.get(i+1)%><BR>
				<%}%>
				</FONT>
			</TD>
		</TR>
...
<TABLE cellspacing="0" cellpadding="0" width="100%" border="0">
	<TBODY>
		<TR valign="top">
			<TD width="40%"><A href="TipoAsunto.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" target="_self"> <IMG
				height="26" src="../Imagenes/nuevo1.gif" width="27" border="0"
				title="Nuevo Tipo de Asunto"> </A> <FONT face="Tahoma" size="2"></FONT> <A
				href="javascript:Borrar()" target="_self"><IMG border="0"
				src="../Imagenes/borrar1.gif" width="30" height="27"
				title="Borrar Tipos de Asunto"></A></TD>
			<TD width="50%">
			<DIV align="right">
				<% 
...
<TABLE width="100%" border="0">
	<TBODY>
		<TR bgcolor="#00204f">
			...
			<b><FONT size="2" color="white" face="verdana">Tipo de Asunto</FONT></b>
...
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
	<% 	int numeroPaginas = numeroRegistros/regPorPagina;	
		...
		String [][] strDatos = 	ActualizaTipoAsunto.getListaTipoAsuntos(strOrden, strTipo);
	...
	<% if(	BUsuario.getAdmon() ||
			BUsuario.getIdArea().equals(Lista7)){ %>
	<% t++; %>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="50%"><INPUT type=checkbox value="<%=Lista1 %>"
						...
						<A href="TipoAsunto.jsp?accion=consultar&TipoAsunto_idTipoAsunto=<%=Lista1 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" 
							title="<%=Lista2%>"><script>document.write(trunc('<%=Lista2%>','Asunto_HD'));</script></A>
					</FONT></td>
					<td width="50%" align="center" valign="middle"><a title="<%=Lista3%>">
					<script>document.write(trunc('<%=Lista3%>','Asunto_HD'));</script></a></td>
				</tr>
			</table>
			</TD>
		</TR>
	...
	<TR>
		<TD width="100%">
		<div align=center><FONT face="verdana" size="4">******No hay Tipo de Asuntos
