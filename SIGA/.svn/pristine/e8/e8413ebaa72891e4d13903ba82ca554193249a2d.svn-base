...
<TABLE border="0" width="70%">
		...
		%>
				...
				<UL><LI>
					<% if (ActualizaUsuario.getAreaUsuario(strIdMando).equals(strIdArea)){ %>
						<A href="estructuraPuestoGestion.jsp?id=<%=strIdMando%>&area=<%=strIdArea%>&retURI=<%=Utilerias.codificaPostHTML(strTmp+"?id="+strIdUsuario+"&area="+strIdArea+"&retURI="+Utilerias.codificaPostHTML(retURI))%>"
	...
	<tr>
		...
		<td align="left" width="696" class="etiquetas">
			...
			<A href="estructuraPuestoGestion.jsp?id=<%=asistentes[i]%>&area=<%=strIdArea%>&retURI=<%=Utilerias.codificaPostHTML(strTmp+"?id="+strIdUsuario+"&area="+strIdArea+"&retURI="+Utilerias.codificaPostHTML(retURI))%>"
				class="etiquetas" title="<%= ActualizaUsuario.NombreUsuario(asistentes[i])%>"><%= ActualizaUsuario.NombrePuesto(asistentes[i])%></A>
			<%}%>
		</td>
		<% }else{ %>
		<TD align="left" width="696" class="etiquetas">
	...
	<tr>
		...
		<% if(strCompanieros.length > 0){ %>
			<% for(int i=0;i<strCompanieros.length;i++){ %>
				...
				%>
			<%		if (bandera){ %>
			<A href="estructuraPuestoGestion.jsp?id=<%=strCompanieros[i][0]%>&area=<%=strIdArea%>&retURI=<%= Utilerias.codificaPostHTML(strTmp+"?id="+strIdUsuario+"&area="+strIdArea+"&retURI="+Utilerias.codificaPostHTML(retURI)) %>" class="etiquetas"
