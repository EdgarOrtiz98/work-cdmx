...
<%
	try{
		response.sendRedirect("estructura.jsp");
	}
	catch(java.io.IOException io){;}
	...
	String sql[][] = ActualizaUsuario.getUsuariosPorArea(strIdArea, strOrden, strTipo);
...
<FORM action="UsuariosView.jsp" name="UsuariosView" method="post">
...
<INPUT type="hidden" value="0" name="contPag">
<INPUT type="hidden" value="0" name="contligas">
<%if(vTiene.size()>0){%>
...
<TABLE cellspacing="0" cellpadding="0" width="100%" border="0">
	<TBODY>
		<TR valign="top">
			<TD width="40%"><A onClick="referenciaRet(document.UsuariosView.retURI);" target="_self"> <IMG
				height="26" src="../Imagenes/nuevo1.gif" width="27" border="0"
				title="Nuevo Puesto"> </A> <FONT face="Tahoma" size="2"></FONT> <A
				href="javascript:Borrar()" target="_self"><IMG border="0"
				src="../Imagenes/borrar1.gif" width="30" height="27"
				title="Borrar Puesto(s)"></A>
			</TD>
			<TD width="50%">
			<DIV align="right">
				<% 
...
<TABLE width="100%" border="0">
	<TBODY>
		<TR bgcolor="#00204f">
			...
			<TD id="Asunto_HD2" width="25%" align="center">
...
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0"> 
	<% int j = 0; 
		...
		String strDatos[][] = ActualizaUsuario.getUsuariosPorArea(strIdArea, strOrden, strTipo);		
		int numeroPaginas = numeroRegistros/regPorPagina;	
		if (numeroRegistros%regPorPagina != 0)
			numeroPaginas++; 
...
%>
	...
	<% t++; %>
		<TR valign="top" <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="65%"><INPUT type=checkbox value="<%=Lista1 %>"
						name="Borrar"><FONT face=Arial color=#000080 size=1> 
					<A <% if (BUsuario.getSupervisor() && strRolLIsta.equals("0")){} else{ %>href="Usuarios.jsp?accion=consultar&id_usuario=<%=Lista1 %>&retURI=<%=Utilerias.codificaPostHTML(strTmp+"?regIDArea="+strIdArea+"&orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" <%}%> title="<%=Lista2%>"><script> document.write(trunc('<%=Lista2%>','Asunto_HD'));</script></A>
					</FONT></td>
					<td width="35%" align="center"><A onmouseover="title='<%=Lista5%>'"> <script> document.write(trunc('<%=Lista5%>','Asunto_HD'));</script></A></td>
				</tr>
			</table>
			</TD>
		</TR>
