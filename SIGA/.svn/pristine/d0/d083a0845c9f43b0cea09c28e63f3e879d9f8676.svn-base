...
Calendar.prototype.show = function() {
	...
	this.wwrite("</head>");
	this.wwrite("<body onBlur='window.focus()' LEFTMARGIN='0' TOPMARGIN='0' MARGINWIDTH='0' MARGINHEIGHT='0' BACKGROUND='./Imagenes/fondo_claro.jpg'" + 
...
function show_calendar() {
	...
	*/
	...
	vWinCal = window.open("", "Calendar", features+((features!='')?',':'')+"width=250,height=200,status=no,resizable=no,modal=no,scrollbars=yes");
	vWinCal.opener = self;
	ggWinCal = vWinCal;
	Build(p_item, p_month, p_year, p_format);
}
function show_calendarExp() {
	/* 
		p_month : 0-11 for Jan-Dec; 12 for All Months.
		p_year	: 4-digit year
		p_format: Date format (mm/dd/yyyy, dd/mm/yy, ...)
	...
	*/
	...
	if (arguments[3] == null)
		...
		p_format = arguments[3];
	vWinCal = window.open("", "Calendar", features+((features!='')?',':'')+"width=250,height=200,status=no,resizable=no,modal=no");
	vWinCal.opener = self;
	ggWinCal = vWinCal;
	Build(p_item, p_month, p_year, p_format);
	anio();
