...
<%
	...
	bExistenRem = ActualizaReportes.getExistenEstatus();
...
<FORM name="viewEst" action="ViewEstatus.jsp" method="post">
...
<%--
<DIV align="right">
<IMG border="0" src="../Imagenes/tit_por_est.gif" width="244" height="32">
</DIV>--%>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr valign="top">
	<td width="43%">
		<%if(BUsuario.getSupervisor()){%>
		<a href="ReporteEstatus.jsp?retURI=<%=com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas)%>" title="Genera Reporte">
	...
	<font face="verdana" size="4">Asuntos por Estatus</font>
	</td>
</tr>
</table>
...
<TABLE WIDTH='100%' BORDER=0>
	...
	<TR>
		...
		<%	numRegistros = ActualizaReportes.getReportesPorEstatus(strOrden,strTipo,BUsuario.getIdArea()).length;
			if (numRegistros>0){ %>
			<!-- &nbsp;&nbsp;&nbsp;&nbsp;
	...
	<%	String[][] sql = ActualizaReportes.getReportesPorEstatus(strOrden,strTipo,BUsuario.getIdArea());
	...
	<TR <%if(par(ix+1)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD WIDTH='15%' align="center">
			...
			<%}%>
			<A href="ReporteEstatus.jsp?id_rep=<%=campo1%>&retURI=<%=com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas)%>">
				<%=campo1%>
			</A>
			</font>
		</TD>
