...
<%
	...
	bExiste					=	ActualizaExpArchivado.getExisteFon();	
	...
	{
		bOperacion1	=	ActualizaExpArchivado.getDeleteFondo(strIds_Borrar,BUsuario.getIdUsuario());	
			if(bOperacion1 == 2)
			{
...
<FORM action="FondoArchview.jsp" name="fondview" method="post">
...
<TBODY>
	<TR valign="top">
		<TD width="43%">
			...
			<A href="FondoArch.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>">
				<IMG height="26" src="../Imagenes/nuevo1.gif" title="Agregar Fondo" width="27" border="0"></A>
				<FONT face="Tahoma" size="2">
				</FONT>
				<A href="javascript:Borrar()">
				<IMG border="0" title="Borrar Fondo" src="../Imagenes/borrar1.gif" width="30" height="27">
				</A>
		</TD>
		<TD width="57%" align="right" valign="middle">
		<%	numRegistros = ActualizaExpArchivado.getDatosFondo(strOrden, strTipo).length; %>
...
<TBODY align="center">
	<TR bgcolor="#00204f">
		<TD align="center" >&nbsp;
					<FONT size="2" color="white" face="verdana">CATALOGO FONDOS</FONT>
		</TD>
	</TR>
	<TR valign="top">
		<TD width="100%" colspan="2"></TD>
...
<TBODY>
	<TR bgcolor="#00204f">
		...
		<TD id="Asunto_HD" width="30%" align="center">
			...
			<b><FONT size="2" color="white" face="verdana">Nombre Fondo</FONT></b>
...
<TBODY>
	<% if(bExiste){ %>
		<%	String[][] sql = ActualizaExpArchivado.getDatosFondo(strOrden, strTipo);
	...
	<TR vAlign="middle" <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="15%">
		    ...
		    <A href="FondoArch.jsp?id_sub=<%= campo1 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>"><%= campo4 %></A>
    		</FONT>
	   	</TD> 
	...
	<TR> 
		<TD width="100%">
			<div align=center>
				<FONT face="verdana" size="4">******No Hay Fondos capturados******</FONT>
			</div>
		</TD>
	</TR>
	<% } %>
