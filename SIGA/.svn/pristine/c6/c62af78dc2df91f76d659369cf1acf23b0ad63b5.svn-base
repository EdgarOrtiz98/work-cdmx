...
if(core.doSplash){
	document.write('<div id="domapi_splash" style="background-color:infobackground;color:infotext;position:absolute;'+(navigator.userAgent.indexOf("MSIE")>0?'right:18px;':'right:2px;')+'top:3px;white-space:nowrap;border:1px solid threedshadow;font:8pt arial,helvetica,sans-serif;padding:0px 3px;">DomAPI '+core.version+'</div>')
	};
core.libs=["core"];
...
core._sniff=function(){
	...
	core._checkStrict=function(){
		...
		if(d){
			if(d.systemId)r=d.systemId.indexOf("strict")>-1;
			else if(d.publicId)r=d.publicId.indexOf("transitional")>-1}
		...
		core.addEvent=function(o,t,fn,useCapture){
			if((t=="scroll")&&(!core.isIE)){
				...
				if(!this.onScrollTimer&&core.reflow){
					...
					core.onfiltered=setInterval("if((core.reflow.oldPageYOffset!=pageYOffset)||(core.reflow.oldPageXOffset!=pageXOffset)){for(var i=0;i<core.scrollEvents.length;i++)core.scrollEvents[i]();core.reflow.oldPageYOffset=pageYOffset;core.reflow.oldPageXOffset=pageXOffset};",25)
				}
				return
			}
			...
			else if(core.isDom&&core.isIE5Mac){
				o["on"+t]=fn}
			else 
