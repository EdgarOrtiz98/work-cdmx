...
<TABLE WIDTH="900" align="center">
	<TR>
		...
		<td>
		<font color="#004080" face="Arial"><strong><%=tableManager.getTotalRegistros() %> Instrucciones</strong></font>
		</td>
	</TR>
	<TR BGCOLOR="00204F">
		<logic:iterate id="encabezado" type="com.meve.util.Encabezado"
			name="tableManager" property="encabezados" indexId="index">
          <TD align="center" width="${encabezado.width}%">
             <xg:sortedHeader key="${encabezado.titulo}" id="${index}" form="reporteForm" 
                              orden="<%=tableManager.getOrden(index.intValue())%>" />
		  </TD>
		</logic:iterate>
	</TR>
</TABLE>
...
<TABLE width="900" align="center" >
	<logic:iterate id="registro" name="tableManager" property="registrosPaginaActual"
		...
		<tr>
			<td WIDTH='25%'>${registro.instruccionDescripcion}</td>
			<td WIDTH='20%'>${registro.folioTurnado}</td>
			<td WIDTH='15%'>${registro.fechaAsignacion}</td>
			<td WIDTH='15%'>${registro.usuarioNombre}</td>
			<td WIDTH='15%'>${registro.estatusNombre}</td>
			<td WIDTH='10%'>${registro.avance}</td>
		</tr>
	</logic:iterate>
	<tr>
		<td colspan="6">
