...
<%
	...
	bExistenRem = ActualizaReportes.getExistenRemI(BUsuario.getIdArea());
...
<FORM name="viewRemI" action="ViewRemitenteI.jsp" method="post">
...
<tr valign="top">
	<td width="43%">
		...
		<a href="ReporteRemitenteI.jsp" title="Genera Reporte">
	...
	<font face="verdana" size="4">Asuntos por Remitentes Internos</font>
	</td>
</tr>
</table>
...
<TABLE WIDTH='100%' BORDER=0>
	...
	<TR>
		...
		<%	numRegistros = ActualizaReportes.getReportesPorRemitenteI(BUsuario.getIdArea(),strOrden,strTipo).length;
			if (numRegistros>0){ %>
			<!-- &nbsp;&nbsp;&nbsp;&nbsp;
	...
	<%	String[][] sql = ActualizaReportes.getReportesPorRemitenteI(BUsuario.getIdArea(),strOrden,strTipo);
	...
	<TR <%if(par(ix+1)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD WIDTH='15%' align="center">
			...
			<%}%>
			<A href="ReporteRemitenteI.jsp?id_rep=<%=campo1%>&retURI=<%=com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas)%>">
				<%=campo1%>
			</A>
			</font>
		</TD>
