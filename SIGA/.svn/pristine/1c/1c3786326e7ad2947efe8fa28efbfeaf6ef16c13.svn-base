...
<%
	...
	bExiste					=	ActualizaExpArchivado.getExisteSeccion();	
	...
	{
		bOperacion1	=	ActualizaExpArchivado.getDeleteSeccion(strIds_Borrar,BUsuario.getIdUsuario());	
			if(bOperacion1 == 2)
			{
...
<FORM action="SeccionArchview.jsp" name="seccion" method="post">
...
<TBODY>
	<TR valign="top">
		<TD width="43%">
			...
			<A href="SeccionArch.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>">
				<IMG height="26" title="Agregar Sección" src="../Imagenes/nuevo1.gif" width="27" border="0"></A>
				<FONT face="Tahoma" size="2">
				</FONT>
				<A href="javascript:Borrar()">
				<IMG border="0" title="Borrar Sección" src="../Imagenes/borrar1.gif" width="30" height="27">
				</A>
		</TD>
		<TD width="57%" align="right" valign="middle">
		<%	numRegistros = ActualizaExpArchivado.getDatosSeccion(strOrden, strTipo).length; %>
...
<TBODY align="center">
	<TR bgcolor="#00204f">
		<TD align="center" >&nbsp;
					<FONT size="2" color="white" face="verdana">CATALOGO SECCIONES</FONT>
		</TD>
	</TR>
	<TR valign="top">
		<TD width="100%" colspan="2"></TD>
...
<TBODY>
	<TR bgcolor="#00204f">
        ...
        <TD width="20%" align="center">
			...
			<b><FONT size="2" color="white" face="verdana">Nombre Sección</FONT></b>
        ...
        <TD  id="Asunto_HD2" width="10%" align="center">
...
<TBODY>
	<% if(bExiste){ %>
		<%	String[][] sql = ActualizaExpArchivado.getDatosSeccion(strOrden, strTipo);
	...
	<TR vAlign="middle" <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="15%">
    		<INPUT type=checkbox value="<%= campo5 %>" name="Borrar">
    		<FONT face=Arial color=#000080 size=1>
		    <A href="SeccionArch.jsp?id_sub=<%= campo5 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" class ="etiquetas" title= "<%=campo1 %>"><%= campo2 %></A>
    		</FONT>
	   	</TD> 	
		<TD width="15%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A class ="etiquetas" title= "<%=campo3 %>"><%=campo4%></A>
			</FONT>
		</TD>
		<TD width="15%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A class ="etiquetas"><%=campo6%></A>
			</FONT>
		</TD>		
		<TD width="20%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A title="<%=campo7%>"> <script> document.write(trunc('<%=campo7%>','Asunto_HD'));</script> </A>
			</FONT>
		</TD>
		<TD width="25%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A title="<%=campo8%>"><script> document.write(trunc('<%=campo8%>','Asunto_HD2'));</script></A>
			</FONT>
		</TD>
	...
	<TR> 
		<TD width="100%">
			<div align=center>
				<FONT face="verdana" size="4">******No Hay Secciones capturadas******</FONT>
			</div>
		</TD>
	</TR>
	<% } %>
