...
<%@ page import="com.meve.sigma.beans.*,com.meve.sigma.util.*,java.util.*,com.meve.sigma.beans.search.*"%>
...
<%
	...
	String strEntidad			=	(Asunto.getAsuntoEntidad()== null)?"":new String(Asunto.getAsuntoEntidad().getBytes(), "ISO-8859-1");
	...
	if(strEstatus.equals("0")){
		strEstatusN = "En Recepci&ocuate;n";
	}else if(strEstatus.equals("1")){
		strEstatusN = "Por Turnar";
	}else if(strEstatus.equals("2")){
		strEstatusN = "En tr&acute;mite";
	}else if(strEstatus.equals("3")){
		strEstatusN = "Terminada";
	}%>
...
<INPUT type="hidden" value="<%=strFoco%>" name="focus">
...
<INPUT type="hidden" value="<%=strEstatus%>" name="asuntoEst">
	...
	<TR>	
		...
		<c:choose>
			<c:when test='${BEscritura.value}'>
                        <%-- KHCH No se muestra iconos cuando el asunto esta en tramite--%>
		...
		<c:choose>
			<c:when test='${BEscritura.value}'>
				...
				<TD width="5%">
					<a onclick="if(botonBandera){botonBandera=false; AbrirAnex(<%=strIdAsunto %>,'<%=ids %>','<%=refs %>');}" 
						onMouseOver="document.Refe.src='../Imagenes/asuntAntOff.gif';" 
						onMouseOut="document.Refe.src='../Imagenes/asuntAntOn.gif';">
					<IMG border="0" src="../Imagenes/asuntAntOn.gif" title="Anexar Relaci&oacute;n" name="Refe"></a>
				</TD>
				<%}%>
			</c:when>
			<c:otherwise>
...
<TBODY>
	...
	<TR>
		...
		<TD width="380" align="right">
			<FONT face="Verdana" size="2">
				Folio Identificador: <b><%= strFolioGenerado %></b>
			</FONT>
		</TD>
	</TR>
	<TR>
	...
	<TR valign="top">
		...
		<TD width="380">
			<DIV  align="right">
				<FONT face="Verdana" size="2">Creado por: <I><%= BUsuario.getCargo() %> - <%= strFechaCreacion %></I></FONT>
			</DIV>
		</TD>
	</TR>
	<% } %>
...
<TBODY>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			...
			<FONT face="Arial" size="2"> 
				...
				<INPUT class="blue600a" size="80" name="asuntoEntidad" value="<%= strDatosDes[0][1] %>" onclick="" disabled="disabled">
				<% }else{ %>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			...
			<FONT face="Arial" size="2">
				<% if (strDatosDes.length!=0){ %>
				<INPUT class="blue600a" size="80" name="asuntoRemitente" value="<%= strDatosDes[0][3] %> / <%= strDatosDes[0][2] %>" disabled="disabled">
				<% }else{ %>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
				<INPUT class="blue600a" name="asuntoFirmante" value="<%= strFirmante %>" disabled="disabled">
			<%}else{%>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			...
			<%if(strIdAsunto != null && strIdAsunto.trim().length()>0){ %>
				<%=(strTipoFolio.equals("1")?"Salida":"Entrada")%>
				<input name="asuntoTipoFolio" type="hidden" value="<%=strTipoFolio%>">
			<%}else{%>
				<%if(strTipoFolio.length()==0){%>
				Por Asignar<input name="asuntoTipoFolio" type="hidden" value="<%=strTipoFolio%>">
...
<TBODY>
	...
	<TR valign="top">
		<TD width="183">
			...
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<%}else{%>
			<!-- <INPUT type="button" onclick="document.Asunto.strBandera.value=true;AbrirIframe('pop4', '<%=strIdAsunto%>');" class="s-dropdown-btn"> -->
			...
			<!-- <DIV class="popup" id="pop4">
				<IFRAME src="FrameVentanilla.jsp?idArea=<%=BUsuario.getIdArea()%>&ida=<%=strIdAsunto%>&num=1" WIDTH=600 HEIGHT=270 frameborder=1></IFRAME>
			</DIV> -->
		</TD>
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%><BR>
				<% if (strDatosDes.length!=0){ %>
				<INPUT class="blue600a" size="80" name="asuntoDestinatario" type="text" value="<%= strDatosDes[0][6] %>" disabled="disabled">
				<% }else{ %>
				<INPUT class="blue600a" size="80" name="asuntoDestinatario" type="text" value="<%= strDestinatario %>" disabled="disabled">
				<% } %>
				<input name="asuntoCargoDest" value="<%=strCargoDest%>" type="hidden" disabled="disabled">
				<input name="asuntoAreaDest" value="<%=strAreaDest%>" type="hidden" disabled="disabled">
			<%}else{%><BR>
				<INPUT class="blue600a" size="80" name="asuntoDestinatario" type="text" value="<%= strDestinatario %>" onfocus="blur();">
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
				...
				<OPTION><%= strDatosDesPara[t][0] %></OPTION>
				<%}%>
				</SELECT>
				<%}else{%>
				<SELECT name="listTO" size="8" class="blue600a" multiple="multiple" disabled="disabled">
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%if(strDatosDesCC.length!=0 && (strEstatus.equals("2") || strEstatus.equals("3"))){%>
				...
				<OPTION><%= strDatosDesCC[t][0] %></OPTION>
				<%}%>
				</SELECT>
			<%}else{%>
	...
	<TR valign="top">
		<TD width="183">
			...
			<%}else{%> <INPUT type="button"
				onclick="document.Asunto.strBandera.value=true;AbrirDest('<%=Utilerias.codificaPostHTML(Utilerias.getCadenaParaCC(listaIDSParaExt))%>', '<%=strIdsParaE%>', '<%= strAreaUs %>');"
...
<TBODY>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
				<INPUT class="blue600a" size="80" name="asunto" type="text" value="<%= strAsunto %>" disabled="disabled">
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%	if(strDatosDes.length!=0 && (strEstatus.equals("2") || strEstatus.equals("3"))){ %>
				<INPUT type="text" name="asuntoTipoDocto_2" value="<%=strDatosDes[0][9]%>" class="blue600a" disabled="disabled">
				<INPUT type="hidden" name="asuntoTipoDocto" value="<%=strIdTipoDocto%>">
				<INPUT type="hidden" name="esRecepcion" value="2">
				<INPUT type="hidden" name="asuntoTipoDocto_1" value="<%=strIdTipoDocto%>">
			<%	}else{ %>
			...
			<FONT face="Arial" size="2">
			<meve_combo:comboseleccion data="<%= ComboData.getTipoDoctoAreaCombo(strAreaUs) %>" size="1" name="asuntoTipoDocto_1" classHtml="blue600a" selected="<%= strIdTipoDocto %>" textoNoSeleccion="  " valorNoSeleccion="-1" disabled="true"/>
			</FONT>
			<INPUT type="hidden" name="esRecepcion" value="2">
			<INPUT type="hidden" name="asuntoTipoDocto_1" value="<%=strIdTipoDocto%>">
			<INPUT type="hidden" name="asuntoTipoDocto" value="<%=strIdTipoDocto%>">
	...
	<tr valign="top">
		...
		<TD colspan="3">
			...
			<% }else{ %>
			<INPUT class="blue100a"  name="asuntoFechaRecepcion" value="<%=strFeFormateada1%>" maxlength="10" onblur="validaFormatoFecha();keyTra()" onKeyUp="validaFechaRec();" onKeyPress="AceptaSoloTeclaNumericaYDiagonal();" onkeydown="document.Asunto.strBandera.value=DetectaCambios(document.Asunto.strBandera.value);">
	...
	<tr valign="top">
		...
		<TD colspan="3">
			<%if(strDatosDes.length!=0 && (strEstatus.equals("2") || strEstatus.equals("3"))){%>
			<INPUT name="asuntoTramite_2" type="text" value="<%= strDatosDes[0][11] %>" disabled="disabled" class="blue600a">
			...
			<SELECT name="asuntoTramite" class="blue600a" onchange="document.Asunto.strBandera.value=true;pruebaCom(this.value)">
				...
				<OPTION value="<%=strTramite1[i][0]%>" <%=(strTramite.equals(strTramite1[i][0])?"selected":"")%>><%=strTramite1[i][1]%></OPTION>
				<%}%>
				<OPTION value="-1">------ Selecciona un Tr&aacute;mite General ------</OPTION>
				<%
					String strTramite2[][] = null;
					strTramite2 =  ActualizaTramite.getComboTramiteGral();
					for(int i=0;i<strTramite2.length;i++){					
	...
	<tr valign="top">
		...
		<td colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<INPUT class="blue100"  name="asuntoFechaCad" value="<%= strFechaCaducidad %>" disabled="disabled">
			<% }else{ %>
				<%if(strIdAsunto.length()==0){%>
	...
	<tr valign="top">
		...
		<td colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<INPUT class="blue500a" name="asuntoFolioExt" value="<%= strFolioExterno %>" disabled="disabled">
			<%}else{%>
	...
	<tr valign="top">
		...
		<td colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<INPUT class="blue500a" name="asuntoFolioInt" value="<%= strFolioIntermedio %>" disabled="disabled">
			<%}else{%>
	...
	<TR valign="top">
		<TD width="183">
			<B><FONT face="Arial" color="#004080" size="2">Descripci&oacute;n:</FONT> </B></TD>
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<TEXTAREA name="asuntoSintesis" rows="8" cols="60" disabled="disabled" class="blue600a"
			onKeyDown="limitaTextArea(document.Asunto.asuntoSintesis,'Solicitud','4000')"><%= strSintesis %></TEXTAREA>
			<%}else{%>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			...
			onKeyDown="limitaTextArea(document.Asunto.asuntoComentario,'Comentarios','4000')"><%= strComentarios %></TEXTAREA>
			<%}else{%>
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
				<INPUT class="blue600a" size="80" name="asuntoPalabraClave" value="<%= strPalabraClave %>" disabled="disabled">
			<%}else{%>
				<INPUT class="blue600a" size="80" name="asuntoPalabraClave" value="<%= strPalabraClave %>" maxlength="200" onkeydown="document.Asunto.strBandera.value=DetectaCambios(document.Asunto.strBandera.value);">
			<%}%>
		</TD>
    </TR>
	<tr valign="top">
	...
	<TR valign="top">
		...
		<TD colspan="3">
			<FONT face="Arial" size="2">
				<% if(strIdAsunto != null && strIdAsunto.trim().length()>0){ %>
                                    <%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
				<IFRAME src="ExpedienteInstrccion.jsp?strIdAsunto=<%=strIdAsunto%>" WIDTH=800 HEIGHT=100 frameborder=0 id="Entidad"></IFRAME>
                                    <%}else{%>
...
<TABLE border="0">
	...
	<TR <%if(par(m)==true){%> <%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="438" align="left"><%= strFolioGenerado %>&nbsp;es&nbsp;<%=strDatosAsuntoAnt[m][1]%>&nbsp;de&nbsp; 
	...
	<TR <%if(par(m)==true){%> <%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="438" align="left">
			...
			<A href="AsuntoRecepcion.jsp?id_asunto=<%= strConsulta[0][0] %>&retURI=<%= Utilerias.codificaPostHTML(strTmp+"?id_asunto="+strIdAsunto+"&retURI="+Utilerias.codificaPostHTML(retURI)) %>">
			<FONT size="1" color="#004080" face="Arial"> <%= strConsulta[0][1] %></FONT></A>
			<%}%> &nbsp;es&nbsp;<%=strDatosAsuntoLigas[m][1]%>&nbsp;de&nbsp;<%=strFolioGenerado%>
		</TD>
		<!-- <td width="274" align="center"><FONT size="1" color="#004080"
...
<TBODY>
	...
	<tr valign="top">
		...
		<td colspan="3">
			<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
			<INPUT class="blue100a"  name="fechaEvento" value="<%= strFormateada %>" disabled="disabled">
			<%}else{%>
			<FONT face="Arial" size="2">
			<INPUT class="blue100a"  name="fechaEvento" value="<%= strFormateada %>" maxlength="10" onblur="validaFormatoFecha();" onKeyUp="validaFechaEve();" onKeyPress="AceptaSoloTeclaNumericaYDiagonal();" onkeydown="document.Asunto.strBandera.value=DetectaCambios(document.Asunto.strBandera.value);">
			</FONT> 
	...
	<tr valign="top">
		...
		<td colspan="3">
		<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
		<TEXTAREA name="lugarEvento" rows="2" cols="60" disabled="disabled" class="blue600a"><%=strLugarEvento %></TEXTAREA>
		<%}else{%>
		<TEXTAREA name="lugarEvento" rows="2" cols="60" class="blue600a"
				onkeydown="document.Asunto.strBandera.value=DetectaCambios(document.Asunto.strBandera.value);limitaTextArea(document.Asunto.lugarEvento,'Lugar del Evento', '100');"><%=strLugarEvento %></TEXTAREA>
		<%}%>
		</td>
	</tr>
	<tr valign="top">
		...
		<TABLE cellspacing="0" cellpadding="0" border="0">
			<TBODY>
				...
				<TR valign="top">
					...
					<TD colspan="3"><%if(strDatosDes.length!=0 && (strEstatus.equals("2") || strEstatus.equals("3"))){%>
					<INPUT name="asuntoPrioridad_2" value="<%= strDatosDes[0][13] %>" disabled="disabled" class="blue600a">
					<%}else{%>
						...
						<OPTION value="<%=strPrioridadArea[x][0]%>"
							<%=(strIdPrioridad.trim().equals(strPrioridadArea[x][0])?"selected":"")%>><%=strPrioridadArea[x][1]%></OPTION>
						<% } %>
						<OPTION value="null">------ Prioridades Generales ------</OPTION>
						<% for(int x=0;x<strPrioridadGral.length;x++){ %>
						<OPTION value="<%=strPrioridadGral[x][0]%>"
							<%=(strIdPrioridad.trim().equals(strPrioridadGral[x][0])?"selected":"")%>><%=strPrioridadGral[x][1]%></OPTION>
						<% } %>
						</SELECT> </FONT>
				...
				<TR valign="top">
					...
					<TD colspan="3"><%if(strDatosDes.length!=0 && (strEstatus.equals("2") || strEstatus.equals("3"))){%>
					<FONT face="Arial" size="2"> 
					<INPUT name="asuntoTipoAsunto_2" value="<%= strDatosDes[0][12] %>" disabled="disabled" class="blue600a">
					</FONT>
				<%}else{%> 
						<FONT face="Arial" size="2"> 					
						...
						<OPTION value="<%=strTipoAsuntoArea[c][0]%>"
							<%=(strIdTipoAsunto.trim().equals(strTipoAsuntoArea[c][0])?"selected":"")%>><%=strTipoAsuntoArea[c][1]%></OPTION>
						<% } %>
						<OPTION value="null">------ Tipos de Asunto Generales ------</OPTION>
						<% for(int c=0;c<strTipoAsuntoGral.length;c++){ %>
						<OPTION value="<%=strTipoAsuntoGral[c][0]%>"
							<%=(strIdTipoAsunto.trim().equals(strTipoAsuntoGral[c][0])?"selected":"")%>><%=strTipoAsuntoGral[c][1]%></OPTION>
						<% } %>
						</SELECT> </FONT>
				<%}%>
				...
				<%--
		<TR valign="top">
		<TD width="183"><FONT face="Arial" color="#004080" size="2">Ruta
		Automático:</FONT></TD>
		<TD colspan="3">
		<%if(strEstatus.equals("2") || strEstatus.equals("3")){%>
		<FONT face="Arial" size="2">
		<meve_combo:comboseleccion data="<%= ComboData.getFlujoAreaCombo(strAreaUs) %>" size="1" 
									name="asuntoRutaAuto" classHtml="blue500" selected="<%= strIdRutaAuto %>" 
									textoNoSeleccion="  " valorNoSeleccion="null" disabled="true"/> 
		</FONT>
		<% }else{ %>
		<FONT face="Arial" size="2">
		<%if(strIdTipoDocto.length() == 0){%>
		<SELECT name="asuntoRutaAuto" class="blue500">
			<OPTION value="null">--Ninguno--</OPTION>
		</SELECT>
		<% }else{ %>
			
			<meve_combo:comboseleccion 	data="<%= ComboData.getFlujoAreaCombo(strIdTipoDocto) %>" 
									size="1" name="asuntoRutaAuto" classHtml="blue500" 
									selected="<%= strIdRutaAuto %>" textoNoSeleccion="-- Selecciona una Ruta Automatica --" 
...
</c:choose> <%if(strIdAsunto != null && strIdAsunto.length() > 0){%> 
<DIV align="center">
<TABLE cellspacing="0" cellpadding="0" border="0">
<TBODY>
	<TR valign="top">
		<TD width="183">
			<B><FONT face="Arial" color="#004080" size="2">Ruta de atenci&oacute;n</FONT></B>
		</TD>
		<td width="233"><img src="../Imagenes/ecblank.gif" border="0"
			height="1" width="1" title=""></td>
		<td width="67"><img src="../Imagenes/ecblank.gif" border="0"
			...
			<TD colspan="4">
				<div>
					<% if(BUsuario.getIdUsuario().equals(strIdDestinatario)){ %>
						...
						<IFRAME ID="frame1" SRC="ImportarCMO.jsp?IdAsunto=<%=strIdAsunto%>&bo=true&est=<%=strEstatus%>" WIDTH="800" HEIGHT="250" frameborder=0></IFRAME> 
						<% }else{ %>
						<IFRAME ID="frame1" SRC="ImportarAR.jsp?IdAsunto=<%=strIdAsunto%>&bo=true&est=<%=strEstatus%>" WIDTH="800" HEIGHT="250" frameborder=0></IFRAME>
						<% } %>
					<% }else{ %>
...
</SCRIPT> <% 	if(BUsuario.getTurnado()){ %> <SCRIPT
	type="text/javascript">
<!--
	doc = document.Asunto;
	if (doc.idDestinatario.value == ""){
		doc.idDestinatario.value = "<%= BUsuario.getIdUsuario() %>";
		doc.asuntoDestinatario.value = "<%= BUsuario.getCargo() %>"+" / "+"<%= BUsuario.getNombre() %>";
		doc.asuntoCargoDest.value = "<%= BUsuario.getCargo() %>";
		doc.asuntoAreaDest.value = "<%= ActualizaArea.NombreArea(BUsuario.getIdArea()) %>";
		doc.mailDest.value = "<%= BUsuario.getCorreo() %>";
	}
//-->
</SCRIPT> <%	} %> </FORM>
</BODY>
