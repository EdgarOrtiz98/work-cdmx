...
<%
	...
	bExiste					=	ActualizaExpArchivado.getExisteClsInf();	
	...
	{
		bOperacion1	=	ActualizaExpArchivado.getDeleteClasifInf(strIds_Borrar,BUsuario.getIdUsuario());
			if(bOperacion1 == 2)
			{
...
<FORM action="ClasifInfArchview.jsp" name="clasifw" method="post">
...
<TBODY>
	<TR valign="top">
		<TD width="43%">
			...
			<A href="ClasifInfArch.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>">
				<IMG height="26" title="Agregar Clasificación" src="../Imagenes/nuevo1.gif" width="27" border="0"></A>
				<FONT face="Tahoma" size="2">
				</FONT>
				<A href="javascript:Borrar()">
				<IMG border="0" title="Borrar Clasificación" src="../Imagenes/borrar1.gif" width="30" height="27">
				</A>
		</TD>
		<TD width="57%" align="right" valign="middle">
		<%	numRegistros = ActualizaExpArchivado.getDatosClasiInf(strOrden, strTipo).length; %>
...
<TBODY align="center">
	<TR bgcolor="#00204f">
		<TD align="center" >&nbsp;
					<FONT size="2" color="white" face="verdana">CATALOGO CLASIFICACIONES DE LA INFORMACIÓN</FONT>
		</TD>
	</TR>
	<TR valign="top">
		<TD width="100%" colspan="2"></TD>
...
<TBODY>
	<TR bgcolor="#00204f">
		...
		<TD id="Asunto_HD" width="30%" align="center">
			...
			<b><FONT size="2" color="white" face="verdana">Nombre Clasificación</FONT></b>
...
<TBODY>
	<% if(bExiste){ %>
		<%	String[][] sql = ActualizaExpArchivado.getDatosClasiInf(strOrden, strTipo);
	...
	<TR vAlign="middle" <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="15%">
		    ...
		    <A href="ClasifInfArch.jsp?id_sub=<%= campo1 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>"><%=campo4%></A>
    		</FONT>
	   	</TD> 
		...
		<TD width="40%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A title="<%=campo2%>"> <script>document.write(trunc('<%=campo2%>','Asunto_HD2'));</script> </A>
			</FONT>
		</TD>
		<TD width="15%" align="center">
			<FONT face=Arial color=#000080 size=1>
	...
	<TR> 
		<TD width="100%">
			<div align=center>
				<FONT face="verdana" size="4">******No Hay Clasificaciones capturados******</FONT>
			</div>
		</TD>
	</TR>
	<% } %>
