...
<% if(	!BUsuario.getAdmon() && 
		((BUsuario.getAtencion() 	&& !BUsuario.getSupervisor()) || 
...
<%
	...
	bExiste					=	ActualizaUbicacion.getExiste();	
	...
	{
		bOperacion	=	ActualizaUbicacion.deleteUbicacion(strIds_Borrar, BUsuario.getIdUsuario());
	}
	...
	String[][] sql = ActualizaUbicacion.getListaUbicacion(strOrden, strTipo);
...
<FORM action="UbicacionView.jsp" name="UbicacionView" method="get">
...
<TBODY>
	<TR valign="top">
		<TD width="43%">
			...
			<A href="Ubicacion.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" target="_self">
				<IMG height="26" src="../Imagenes/nuevo1.gif" width="27" border="0" title="Nueva Ubicación">
			</A>
			<A href="javascript:Borrar()">
			<IMG border="0" src="../Imagenes/borrar1.gif" width="30" height="27" title="Borrar Ubicaciones">
			</A>
		</TD>
...
<TBODY align="center">
	<TR bgcolor="#00204f">
		<TD align="center" >&nbsp;
					<FONT size="2" color="white" face="verdana">CATALOGO DE UBICACIÓNES</FONT>
		</TD>
	</TR>
	<TR valign="top">
		<TD width="100%" colspan="2"></TD>
...
<TBODY>
	<TR bgcolor="#00204f">
		<TD id="Asunto_HD" width="50%" align="center">
			...
			<b><FONT size="2" color="white" face="verdana">Ubicaciones</FONT></b>
...
<TABLE width="100%" border="0">
<TBODY>
	<% if(bExiste && vRegistros.size()>0){ %>
		<%	sql = ActualizaUbicacion.getListaUbicacion(strOrden, strTipo);
		...
		%>
	<% if(	BUsuario.getAdmon() || BUsuario.getIdArea().equals(campo3)){ %>
	<% t++; %>
	<TR vAlign=top <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD width="50%">
    		<INPUT type=checkbox value="<%= campo1 %>" name="Borrar">
    		<FONT face=Arial color=#000080 size=1>
		    <A href="Ubicacion.jsp?id_ubi=<%= campo1 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" title="<%= campo2 %>"><script>document.write(trunc('<%=campo2%>','Asunto_HD'));</script></A>
    		</FONT>
	   	</TD>
		<TD width="50%" align="center">
			<FONT face=Arial color=#000080 size=1>
			<A title="<%=campo4%>"> <script>document.write(trunc('<%=campo4%>','Asunto_HD2'));</script> </A>
			</FONT>
		</TD>
	</TR>
	<% j++; %>
	...
	<TR>
		<TD width="100%">
		<div align=center><FONT face="verdana" size="4">******No hay Alguna Ubicación
		Capturada en su Unidad Administrativa ******</FONT></div>
		</TD>
	</TR>
	<% }else{ %>
		<TR>
	...
	<TR>
		<TD width="100%">
			<div align=center>
				<FONT face="verdana" size="4">******No Hay Ubicaciones Capturadas******</FONT>
			</div>
		</TD>
	</TR>
	<% } %>
