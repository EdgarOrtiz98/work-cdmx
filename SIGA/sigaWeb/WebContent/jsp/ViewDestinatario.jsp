...
%>
<%
	boolean bExistenDes = false;
	bExistenDes = ActualizaReportes.getExistenDes(BUsuario.getIdArea());
...
<FORM name="viewDes" action="ViewDestinatario.jsp" method="post">
...
<%--<DIV align="right">
<IMG border="0" src="../Imagenes/tit_asu_des.gif" width="253" height="32">
</DIV>--%>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
<tr valign="top">
	<td width="43%">
		<%if(BUsuario.getSupervisor()){%>
		<a href="ReporteTDest.jsp" title="Genera Reporte">
	...
	<font face="verdana" size="4">Asuntos Por Ventanilla</font>
	</td>
</tr>
</table> 
...
<TABLE WIDTH='100%' BORDER=0>
	...
	<TR>
		...
		<%	numRegistros = ActualizaReportes.getReportesPorDestinatario(BUsuario.getIdArea(),strOrden,strTipo).length;
			if (numRegistros>0){ %>
			<!-- &nbsp;&nbsp;&nbsp;&nbsp;
	...
	<%	String[][] sql = ActualizaReportes.getReportesPorDestinatario(BUsuario.getIdArea(),strOrden,strTipo);
	...
	<TR <%if(par(ix+1)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD WIDTH='15%' align="center">
			...
			<%}%>
			<A href="ReporteTDest.jsp?id_rep=<%=campo1%>&retURI=<%=com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas)%>">
				<%=campo1%>
			</A>
			</font>
		</TD>
