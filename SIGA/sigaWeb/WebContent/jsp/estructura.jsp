...
<TABLE border="0">
	...
	<TR align="center">
		<TD align="center" width="700"><meve_combo:comboseleccion
	...
	<TBODY>
		<TR valign="middle">
			...
			<% if (BUsuario.getAdmon() || BUsuario.getSupervisor()){ %>
				<A href="Usuarios.jsp?retURI=<%= Utilerias.codificaPostHTML(strTmp+"?area="+strIdArea) %>" target="_self"> 
			...
			<TD width="40%" align="right" class="etiquetas" valign="bottom">
				<A href="UsuariosRolView.jsp?regIDArea=<%= strIdArea %>" title="Usuarios ordenados por Roles"><B><U>Usuarios por Roles</U></B></A>
			</TD>
		</TR>
		<TR valign="middle">
...
<TABLE border="0" width="70%">
	...
	<tr>
		<TD align="left" width="696" class="etiquetas">
			<A href="estructuraPuesto.jsp?id=<%= strIdSupervisor %>&area=<%= strIdArea %>&retURI=<%= Utilerias.codificaPostHTML(strTmp+"?area="+strIdArea) %>" class="etiquetas" 
		...
		<TD align="left" width="696" class="etiquetas">
			<% 	int jx = 0;
				...
				<% for(int j=0;j<strMiembros.length;j++){ %>
					...
					%>
				<%		if (bandera){ %>
					<A href="estructuraPuesto.jsp?id=<%=strMiembros[j][0]%>&area=<%=strIdArea%>&retURI=<%= Utilerias.codificaPostHTML(strTmp+"?area="+strIdArea) %>" class="etiquetas" 
