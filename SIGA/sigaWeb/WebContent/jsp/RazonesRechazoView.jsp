...
<%
	...
	if (strAccion != null && strAccion.trim().length() > 0 && strAccion.equals("borrar")) {
		bOperacion = ActualizaRazonRechazo.deleteRazonRechazo(strIds_Borrar, BUsuario.getIdUsuario());
	}
	...
	String[][] sql = ActualizaRazonRechazo.getListaRazonRechazos(strOrden, strTipo);
...
<script language="JavaScript" type="text/javascript">
	function cambio(o, t){
		doc = document.RazonesRechazoView;
...
<FORM action="RazonesRechazoView.jsp" name="RazonesRechazoView" method="get">
...
<TABLE cellspacing="0" cellpadding="0" width="100%" border="0">
	<TBODY>
		<TR valign="top">
			<TD width="40%"><A href="RazonesRechazo.jsp?retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" target="_self"> <IMG
				height="26" src="../Imagenes/nuevo1.gif" width="27" border="0"
				title="Nueva Razón de Rechazo"> </A> <FONT face="Tahoma" size="2"></FONT> <A
				href="javascript:Borrar()" target="_self"><IMG border="0"
				src="../Imagenes/borrar1.gif" width="30" height="27"
				title="Borrar Razón de Rechazo"></A></TD>
			<TD width="50%">
			<DIV align="right">
				<% 
...
<TABLE width="100%" border="0">
	<TBODY>
		<TR bgcolor="#00204f">
			...
			<b><FONT size="2" color="white" face="verdana">Razón Rechazo</FONT></b>
...
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<% 	
		...
		String [][] strDatos = 	ActualizaRazonRechazo.getListaRazonRechazos(strOrden, strTipo);
	...
	<% if(	BUsuario.getAdmon() ||
			BUsuario.getIdArea().equals(Lista6)){ %>
	<% t++; %>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td width="70%"><INPUT type=checkbox value="<%=Lista1 %>"
						...
						<A href="RazonesRechazo.jsp?accion=consultar&Razon_IdRazon=<%=Lista1 %>&retURI=<%= com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas) %>" 
							title="<%=Lista3%>"><script>document.write(trunc('<%=Lista3%>','Asunto_HD'));</script></A>
					</FONT></td>
					<td width="30%" align="center" valign="middle"><A title="<%=Lista2%>"><script>document.write(trunc('<%=Lista2%>','Asunto_HD2'));</script></A></td>
				</tr>
			</table>
			</TD>
		</TR>
	...
	<TR>
		<TD width="100%">
		<div align=center><FONT face="verdana" size="4">******No Hay Razones de Rechazo
