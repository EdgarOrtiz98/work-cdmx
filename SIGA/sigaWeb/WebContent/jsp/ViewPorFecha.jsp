...
<%
	...
	bExistenTD = ActualizaReportes.getExistenRPorFechaC(BUsuario.getIdArea());
...
<FORM name="viewPFCap" action="ViewPorFecha.jsp" method="post">
...
<tr valign="top">
	<td width="43%">
		...
		<a href="ReportePorFecha.jsp" title="Genera Reporte">
	...
	<font face="verdana" size="4">Asuntos por Fecha de Recepción</font>
	</td>
</tr>
</table>
...
<TABLE WIDTH='100%' BORDER=0>
	...
	<TR>
		...
		<%	numRegistros = ActualizaReportes.getReportesPorFechaCaptura(BUsuario.getIdArea(),strOrden,strTipo).length;
			if (numRegistros>0){ %>
			<!-- &nbsp;&nbsp;&nbsp;&nbsp;
	...
	<%	String[][] sql = ActualizaReportes.getReportesPorFechaCaptura(BUsuario.getIdArea(),strOrden,strTipo);
	...
	<TR <%if(par(ix+1)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
		<TD WIDTH='15%' align="center">
			...
			<%}%>
			<A href="ReportePorFecha.jsp?id_rep=<%=campo1%>&retURI=<%=com.meve.sigma.util.Utilerias.codificaPostHTML(strTmp+"?orden="+strOrden+"&tipo="+strTipo+"&contPag="+contPag+"&contligas="+contligas)%>">
				<%=campo1%>
			</A>
			</font>
		</TD>
