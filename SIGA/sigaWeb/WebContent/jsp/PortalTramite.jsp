...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	import="com.meve.sigma.actualiza.*,com.meve.sigma.beans.*,com.meve.sigma.reportes.*,java.util.*,
                gob.hacienda.cgtic.siga.actualiza.AtenderInstrucciones" 
%>
		...
		<TBODY>
			...
			<TR>
				...
				<%	numRegistros = ActualizaInstrucciones.getInstruccionTramiteUsuario(strIdsAsiste[i][0],strOrden,strTipo).length;
					if (numRegistros>0){ %>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<A href="../ReporteAsuntoExcel?tip=inst_tramite_01&id_remitente=<%= strIdsAsiste[i][0] %>">
			...
			<tr valign="top" <%if(par(t)==true){%><%}else{%>bgcolor="#f6f6f6"<%}%>>
				...
				<td align="center" width="30%">
					<a title="<%= usuario7 %><% if(usuario2.length() == 0){ %>(<%= usuario9 %>)
						<% }else{ %>(<%=ActualizaInst.getNombreInstruccion1(usuario2)%>)<% } %>">
					<font size="1" color="#000080" face="Arial">
						<%= Utilerias.recortaString(usuario7,20) %><% if(usuario2.length() == 0){ %>(<%= Utilerias.recortaString(usuario9,20) %>)
		...
		<TBODY>
			...
			<TR>
				...
				<%	numRegistros = ActualizaInstrucciones.getInstruccionTramiteUsuario(strDatos[i][0],strOrden,strTipo).length;
					if (numRegistros>0){ %>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<A href="../ReporteAsuntoExcel?tip=inst_tramite_01&id_remitente=<%= strDatos[i][0] %>">
					<img src="../Imagenes/button_xls.gif" border="0" height="16" width="16">
					<FONT face="Arial" size="2">Exportar a Excel</FONT>
					</A>
				<%	} %>
