		...
		<TR>      
			...
			<TD align="left" width="70%">
				...
				%>
					...
					<OPTION value="sinexp" <%=(strIdArea.trim().equals("sinexp")?"selected":"")%>>-- Expedientes sin Instrucción --</OPTION>
					<OPTION value="todo" <%=(strIdArea.trim().equals("todo")?"selected":"")%>>-- Expedientes con Instrucción --</OPTION>
					<OPTION value="todoexp" <%=(strIdArea.trim().equals("todoexp")?"selected":"")%>>-- Todos Expedientes --</OPTION>
					<OPTION value="sin" <%=(strIdArea.trim().equals("sin")?"selected":"")%>>-- Instrucción sin Expediente  --</OPTION>
					<%
					...
					<OPTION value="<%=strExpedienteGral[z][0]%>" <%=(strIdArea.trim().equals(strExpedienteGral[z][0])?"selected":"")%>><%=strExpedienteGral[z][1]%></OPTION>
					<% } %>	
				</SELECT>
				<%}else{%>
	...
	<% int j = 0; %>
		...
		%>
			...
			<meve_front:ListaDatos
				datos="<%=ActualizaExpArchivado.getporExpedienteInstruccion(Lista1, strOrden, strTipo)%>" campo="usuario">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td width="30%" align="center">
					<!-- <a href="InstruccionesAtender.jsp?id_ins=<%=usuario1%>" title="<%=usuario3%>"><%=com.meve.sigma.util.Utilerias.recortaString(usuario3,30)%></a> -->
					<a title="<%=usuario3%>"><%=com.meve.sigma.util.Utilerias.recortaString(usuario3,40)%></a>
					</td>
					<td width="30%" align="center">
					<a title="<%=usuario4%>"><%=com.meve.sigma.util.Utilerias.recortaString(usuario4,40)%></a>
					</td>
				</tr>
			</table>
			</TD>
