...
<!-- 
	...
	function habilita2()
	{
		if(document.fondo1AS.inactivo.checked == true)
		{
		document.fondo1AS.fecinact.value = "<%=horafec%>";
		document.fondo1AS.fecinact.disabled = false;
		document.fondo1AS.justif.disabled = false;
		}else
		{
	...
	<TBODY>
		<TR valign="top">
			...
			<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
				<a></a>
			<%	}else{%>	
				<a><img	src="../Imagenes/Save.gif"  title="Guardar" border="0"	onclick="Guardarfondo()"></a>
			<%}%>
			</TD>
			<TD width="125">
...
<TABLE cellspacing="0" cellpadding="0" border="0">
	...
	<TBODY>
		...
		<TR valign="top">
			...
			<B>Clave:</B></FONT><BR>
			<%if(bInsertar1 == 2){ %><B><FONT color="red">*Esta clave ya existe*</FONT> </B><%} %>
			</TD>
			<TD width="528">
				<INPUT name="clave"  value="<%=strClave%>" onkeypress = "AceptaSoloTeclaNumericaYPunto();" onblur="validaPref();"
						<%if(valida == 2) 
						{%>
						disabled="disabled"
						<%
			...
			<TD width="528">
				<textarea name="subSeccion" rows="8" cols="60" 
						<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
							disabled="disabled"
						<%	}%>			
				onKeyDown="limitaTextArea(document.fondo1AS.subSeccion,'Descripción','4000')"><%=strSeccion%></textarea>
			</TD>
		</TR>
		<TR valign="top">
			<TD width="144">&nbsp;</TD>
		...
		<TR valign="top">
			...
			<TD width="528">
					<INPUT name="sisiNum" value="<%=strSisi%>" 
						<%if(strInact.trim().length()>0 && Integer.parseInt(strInact) == 3) {%>
							disabled="disabled"
						<%	}%>			
