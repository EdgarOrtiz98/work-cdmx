...
<meve_front:ListaDatos datos="<%=ActualizaArchivo.getArchivosReferenciaContent(strIdAsunto, strIdUsuario, strInstruccion)%>" campo="campo" >
	<%contar++;%>
	<%if(contar==0){%>
	<TR>
		<TD>
	...
	<TR>
		...
		%>	
		<a href="../DownLoadDicument?ida=<%=campo1%>&fol=<%=strFolio %>&name=<%=campo2 %>">
...
<meve_front:ListaDatos datos="<%=ActualizaArchivo.getArchivosReferenciaContentDocSec(strIdAsunto,strIdUsuario, strInstruccion)%>" campo="campo" >
	<%contar++;%>
	<%if(contar==0){%>
	<TR>
		<TD>
...
<table border="0">
  <tr>
    <td width="616"><B><FONT face="Arial" color="#004080" size="2">
		...
		(Favor de contactar a su administrador)	<%=content %>
		</FONT>
	</td>
  </tr>
</table>
